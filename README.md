# 🥝 Kiwi Karlsen — 온라인 체스 플랫폼

무료 온라인 체스 사이트. FastAPI + 순수 JS(빌드 도구 없음) + 번들 Stockfish.

## 주요 기능
- **대국**: 온라인 대국(WebSocket, Glicko-2), 봇 11단계(ELO 슬라이더), 재대국
- **퍼즐**: 훈련(Elo 레이팅)·일일 퍼즐·러시·실시간 전투
- **시각 훈련**: 30초 좌표/수순 스피드런
- **오프닝**: 3,800여 종 탐색기(게임 수·승률·레이팅 필터) + 7단계 학습 커리큘럼
- **분석**: Stockfish 내장, 11단계 게임 리뷰(정확도·예상 레이팅)
- **통찰**: 레이팅 추이·오프닝 성과·정확도·활동 히트맵 등 12가지 분석
- **커뮤니티**: 친구·DM, 체스 클럽(공지·채팅·클럽 보드), 사용자 검색, 업적 70여 종
- **보안**: scrypt+페퍼, 2FA(TOTP), 토큰 폐기, CSP/HSTS, 데이터 내보내기, 회원 탈퇴
- **UI**: 다크 모드, 보드 테마 6종, 모바일 지원, 접근성
...
## 1. Ctrl+F5 없이도 갱신
버전 감시기만으로는 부족했습니다. 캐시된 옛 HTML 에는 감시기 자체가 없기 때문입니다.
**내비게이션 링크에 버전을 붙였습니다** (`/play.html?v=18`).
URL 이 달라지므로 브라우저가 캐시된 `/play.html` 을 쓰지 않고 새로 받습니다.
로비 한 번만 새로고침하면 이후 모든 페이지가 자동으로 최신이 됩니다.

## 2. 승/패/무 전적 표시
프로필에 승·패·무·승률을 각각 표시하고, 이름 아래에 `12승 · 5패 · 3무` 요약줄을 넣었습니다.

## 3. 분석 평가치 요동 수정
내장 JS 엔진에 **정지 탐색(quiescence)** 이 없어 매 수마다 평가가 ±1.00 씩
튀었습니다(수평선 효과). 교환이 끝날 때까지 잡는 수를 더 보도록 고쳐 안정화했습니다.
또한 Stockfish 엔진이 없으면 분석 페이지에 **경고 배너**를 띄웁니다
(정확한 평가에는 `frontend/assets/engine/stockfish.js` 가 필요합니다).

## 4. 오프닝 176 → **248종**, 전위(transposition) 인식
수순 문자열이 아니라 **국면(position)** 으로 매칭하도록 바꿨습니다.
- `1.d4 d5 2.Bf4` (London) ✅
- `1.d4 Nf6 2.Nf3 d5 3.Bf4` (전위) ✅
- `1.Nf3 d5 2.d4 Nf6 3.Bf4` (레티 순서 전위) ✅
- `1.c4 Nf6 2.d4 e6 3.Nc3 Bb4` → Nimzo-Indian (잉글리시 전위) ✅

런던, 조바바 런던, 콜, 콜-자노스키, 토레, 스톤월, 트롬포프스키, 배리 어택,
블랙마-디머, 에반스 갬빗, 마샬 어택, 액셀러레이티드 드래곤, 카안, 리히터-라우저,
케임브리지 스프링스, 라그노, 보고-인디언 등 실전 오프닝을 대거 추가했습니다.
